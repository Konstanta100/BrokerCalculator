...
const DeleteOperationsByAccountID = `-- name: DeleteOperationsByAccountID :execrows
DELETE FROM investing.operations WHERE account_id = $1
`

func (q *Queries) DeleteOperationsByAccountID(ctx context.Context, accountID string) (int64, error) {
	result, err := q.db.Exec(ctx, DeleteOperationsByAccountID, accountID)
	if err != nil {
		return 0, err
	}
